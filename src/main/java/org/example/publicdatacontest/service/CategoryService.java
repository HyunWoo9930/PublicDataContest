...
public class CategoryService {
    ...
    public MentorCategoryResponse getMentorCategory(UserDetails userDetails) {
        String username = userDetails.getUsername();
        Optional<Mentor> mentor = mentorRepository.findByUserId(username);

        if (mentor.isPresent()) {
            return new MentorCategoryResponse(mentor.get().getMentorCategories());
        } else {
    ...
    public MenteeCategoryResponse getMenteeCategory(UserDetails userDetails) {
        String username = userDetails.getUsername();
        Optional<Mentee> mentee = menteeRepository.findByUserId(username);

        if (mentee.isPresent()) {
            return new MenteeCategoryResponse(mentee.get().getMenteeCategories());
        } else {
            throw new NotFoundException("user not found");
        }
    }
    ...
    public void method() {
        subCategoryRepository.findAll().stream()
                .map(i -> i.getCategory().getSubCategories().add(i));
    }
